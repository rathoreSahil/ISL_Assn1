...
def predict(arr, cov1, cov2, cov3, c):
    ...
    plt.title(f"C{c} classifier on non linear data!")

    plt.show()


df1 = pd.read_csv("q2/data/nl_class1.csv", sep=",", header=None)
df1.columns = ['A', 'B']
df1_cov = df1.cov().to_numpy()
df1_mean = df1.mean().to_numpy()
df1['Y'] = 1

df2 = pd.read_csv("q2/data/nl_class2.csv", sep=",", header=None)
df2.columns = ['A', 'B']
df2_cov = df2.cov().to_numpy()
df2_mean = df2.mean().to_numpy()
df2['Y'] = 2

df3 = pd.read_csv("q2/data/nl_class3.csv", sep=",", header=None)
df3.columns = ['A', 'B']
